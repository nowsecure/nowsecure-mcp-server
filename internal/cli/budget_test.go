...
// GraphQL check catalog with one "ssl" match, and an empty MARI catalog (so
// the MARI cases exercise the skip path).
...
func TestProfileCallListsToolsWithoutToken(t *testing.T) {
	clearTokenEnv(t)
	_, out, _, err := runCLI(t, "profile", "call")
	if err != nil {
		t.Fatalf("profile call (list) should not need a token: %v", err)
	}
	for _, tool := range []string{"list_apps", "get_mari_assessment", "decode_nowsecure_url"} {
...
func TestProfileCallSchema(t *testing.T) {
	clearTokenEnv(t)
	_, out, _, err := runCLI(t, "profile", "call", "get_mari_assessment", "--schema")
	if err != nil {
	...
	if _, _, _, err := runCLI(t, "profile", "call", "no_such_tool", "--schema"); err == nil {
...
func TestProfileCallInvokesTool(t *testing.T) {
	...
	_, out, _, err := runCLI(t, "profile", "call", "--token", "test-token",
		"decode_nowsecure_url", `{"url":"https://app.nowsecure.com/app/android/com.example.app/assessment/987654321/findings/apk_janus"}`)
	if err != nil {
	...
	if _, _, _, err := runCLI(t, "profile", "call", "--token", "test-token", "list_apps", "{oops"); err == nil {
...
func TestProfileBudgetSuite(t *testing.T) {
	...
	// Default budgets pass: Platform cases measure small, MARI cases skip
	// (empty catalog), nothing errors.
